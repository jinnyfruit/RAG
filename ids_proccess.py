...
# 결과 출력
if success:
    print("Exclude IDs loaded successfully.")
else:
    print(f"Error loading file: {error_message}")

if success:
    # 제외 아이디 리스트(풀무원TF팀, 코드클릭, 마음AI 계정)
    exclude_ids = ["sin8616", "hjjun0213", "yeajinyu", "case18", "sejookin", "hjjungl", 
               "dmz2817", "heemis", "yspak82", "ananan", "zanghdnwls2", "tngyupp", "rhksgud",
               "jinnyfruit","st0304","chlwnsgh647","leejamie"]

    # 제외해야 할 아이디를 제외하고 중복 없이 출력할 고객 아이디 리스트 생성
    output_ids = list(set(id for id in input_ids if id not in exclude_ids))
    cnt = 0 
